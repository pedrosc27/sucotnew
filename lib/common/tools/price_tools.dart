...
class PriceTools {
  ...
  static String? getCurrencyFormatted(price, Map<String, dynamic>? rates,
    ...
    var formatCurrency = NumberFormat.currency(locale: 'es_MX', symbol: '', decimalDigits: 2 ?? 0);
  ...
  static String? getCurrencyFormattedModificado(price, Map<String, dynamic>? rates,
      {currency}) {
    Map<String, dynamic> defaultCurrency =
        kAdvanceConfig['DefaultCurrency'] ?? {};

      var precioFormato = NumberFormat.currency(locale: 'es_MX', symbol: '\$', decimalDigits: 2).format(price);
