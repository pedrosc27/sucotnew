  ...
  Future<void> getCategories({
    ...
    try {
      ...
      sortCategoryList(
        ...
        sortingList: remapCategories != null ? null : sortingList,
        categoryLayout: categoryLayout,
      );

      if (remapCategories != null) {
        for (var remapCategory in remapCategories) {
          var category = categoryList[remapCategory['category'].toString()];
          if (category != null) {
            category.totalProduct = 0;
            category.name = remapCategory['name'] ?? category.name;
            category.image = remapCategory['image'] ?? category.image;
            category.parent =
                remapCategory['parent']?.toString() ?? category.parent;
          }
        }
      }
